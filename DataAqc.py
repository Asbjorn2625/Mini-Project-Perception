...
def main():
    ...
    color_fx, color_fy = color_intrinsics.fx, color_intrinsics.fy,
    color_cx, color_cy = color_intrinsics.ppx, color_intrinsics.ppy

    
    np.savez(folder + 'intrinsics.npz', width=w, height=h)
    ...
    while True:
        ...
        if key == ord("s"):
            np.savez(folder + f'verts{image_index}.npz', verts=verts, texcoords=texcoords, color_source=color_source)
            image_index += 1
